...
build_rdk=0
original_kernel_nvram_file="/rom/etc/wlan/kernel_nvram.setting"
...
mfg_remount_defaults()
{
	...
	if [ "$FLASHTYPE" == "eMMC" ]; then
		...
		mount -t ubifs -oremount,$1 ubi:defaults $DEFAULTS_MNT_DIR
	fi
}
...
case "$1" in
	...
	;;
    stop)
	rmmod /lib/modules/*/extra/bcm_knvram.ko
	exit 0
	;;
